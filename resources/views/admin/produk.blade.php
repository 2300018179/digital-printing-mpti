    ...
    <div class="flex flex-1">
        ...
        <main class="flex-1 p-8 space-y-6">
            ...
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('admin.produk.create') }}" class="bg-red-700 hover:bg-red-800 text-white font-bold text-xs px-5 py-2.5 rounded-full shadow-sm transition flex items-center gap-2">
            ...
            <div class="flex flex-wrap gap-4 items-center">
                ...
                <div class="relative">
                    <select id="category-filter" onchange="resetPageAndFilter()" class="appearance-none bg-white border border-red-400 rounded-xl pl-4 pr-10 py-2 text-xs font-semibold text-gray-700 focus:outline-none focus:ring-1 focus:ring-red-500 shadow-sm min-w-[150px]">
                        ...
                        <option value="Papan Informasi">Papan Informasi</option>
                        <option value="Tanda Pengenal">Tanda Pengenal</option>
                    </select>
            ...
            @if(session('success'))
                <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-2.5 rounded-xl text-xs font-semibold">
                    ✅ {{ session('success') }}
                </div>
            @endif
                    ...
                    <table class="w-full text-left border-collapse text-xs" id="product-table">
                        <thead>
                            <tr class="bg-red-50 text-red-700 font-bold border-b border-red-100">
                                ...
                                <th class="p-3 w-24 text-center">Gambar</th>
                                <th class="p-3 w-24 text-center">Nama Produk</th>
                                <th class="p-3 w-32 text-center">Deskripsi</th>
                                <th class="p-3 w-24 text-center">Kategori</th>
                                <th class="p-3 w-24 text-center">Harga</th>
                        ...
                        <tr class="product-row hover:bg-gray-50/50 transition" data-page="1">
                            <td class="p-3 text-center text-gray-400 row-number">{{ $key + 1 }}</td>
                            <td class="p-3">
                                @if($product->image)
                                    <img src="{{ asset('storage/' . $product->image) }}" class="w-12 h-12 object-cover rounded-lg border border-gray-200">
                                @else
                                    <div class="w-12 h-12 bg-gray-100 border border-gray-200 rounded-lg flex items-center justify-center text-gray-400 text-lg font-bold">📄</div>
                                @endif
                            </td>
                            <td class="p-3 font-semibold text-gray-800 product-name">{{ $product->name }}</td>
                            <td class="p-3 text-gray-500 max-w-xs truncate" title="{{ $product->description }}">
                                {{ $product->description }}
                            </td>
                            <td class="p-3 product-category">{{ $product->kategori }}</td>
                            <td class="p-3 font-semibold text-gray-900">Rp {{ number_format($product->price, 0, ',', '.') }}</td>
                            <td class="p-3 text-center"><span class="bg-gray-100 text-gray-700 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase">{{ $product->unit }}</span></td>
                            <td class="p-3 text-center">
                                <span class="product-status {{ $product->status == 'Aktif' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' }} text-[10px] px-3 py-0.5 rounded-full font-bold">
                                    {{ $product->status }}
                                </span>
                            </td>
                            <td class="p-3 text-center">
                                <div class="flex justify-center gap-3 items-center">
                                    <a href="{{ route('admin.produk.edit', $product->id) }}" class="text-blue-600 hover:text-blue-800 text-sm">✏️</a>
                                    <form action="{{ route('admin.produk.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="8" class="text-center py-8 text-xs font-medium text-gray-400 italic">
                                📭 Belum ada data produk di database. Silakan klik tombol "+ Tambah Produk".
                            </td>
                        </tr>
                        @endforelse
                        
                        <tr id="no-data" class="hidden">
                            <td colspan="8" class="text-center py-8 text-xs font-medium text-gray-400">
    ...
    <script>
        ...
        function filterTable() {
            ...
            rows.forEach(row => {
                ...
                const productStatus = row.querySelector('.product-status').textContent.trim();
