...
<body class="font-sans m-0 p-0 box-border text-gray-800 bg-white">
    <div class="fixed top-0 left-0 w-full z-[9999] shadow-[0_4px_10px_rgba(0,0,0,0.1)]">
                                        ...
                                        <form action="{{ route('customer.pembayaran') }}" method="POST" id="formCheckoutPopUp">
                                            ...
                                            <div class="flex flex-col max-h-[340px] overflow-y-auto p-3 gap-3 divide-y divide-gray-100">
                                                @foreach($cartItemsData as $item)
                                                    <div class="flex items-start gap-3 pt-3 first:pt-0 justify-between group min-w-0">
        ...
        <nav class="bg-brandRed h-[50px]">
            <div class="max-w-[1350px] mx-auto px-[15px] w-full flex h-full items-center">
                @if(!Route::is('customer.jam-layanan') && !Route::is('customer.tentang-kami') && !Route::is('customer.detail-produk') && !Route::is('customer.pembayaran') && !Route::is('customer.pesanan') && !Route::is('customer.informasi') && !Route::is('customer.promo') && !Route::is('customer.notifikasi') && !Route::is('customer.pesanan-saya'))
                    <div class="bg-brandRed text-white h-full w-[280px] flex items-center font-bold text-sm rounded-[15px_15px_0_0] gap-3 cursor-default pointer-events-none user-select-none shadow-[6px_0_10px_rgba(0,0,0,0.15)] relative z-10 pl-5">
                        <i class="fa fa-bars mr-3"></i> Pilih Kategori
                    </div>
                @else
                    <div class="w-[280px] h-full hidden md:block"></div>
                @endif
                <ul class="hidden md:flex list-none gap-[35px] ml-6 flex-1 pl-4">
    ...
    <footer class="bg-[#c40000] text-white py-[25px] mt-[50px] text-[13px] [line-height:1.6] font-sans">
        <div class="max-w-[1350px] mx-auto px-[20px] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-[30px] items-start">
            <div class="w-full">
