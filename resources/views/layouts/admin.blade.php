...
<head>
    ...
    <!-- Alpine.js untuk fitur toggle popup profil -->
...
<body class="bg-gray-50 flex flex-col min-h-screen">
    ...
    <header class="bg-white border-b border-gray-200 px-6 py-3 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        ...
        <!-- Bagian Kanan Header (Profile Dropdown Popup) -->
        <div class="relative" x-data="{ open: false }">
            <div class="flex items-center gap-3">
                ...
                <!-- Tombol Avatar Trigger (Foto Profil / Default Icon) -->
                <button @click="open = !open" 
                    ...
                    @php
                        // Logika Cek Foto Profil User
                        $user = Auth::user();
                        $userEmail = $user->email ?? '2300018154@webmail.uad.ac.id';
                        // Menggunakan Gravatar otomatis berdasarkan Hash Email jika tidak ada upload foto lokal
                        $gravatarUrl = 'https://www.gravatar.com/avatar/' . md5(strtolower(trim($userEmail))) . '?d=404';
                        
                        // Prioritas foto: 1. Field foto DB (jika ada) -> 2. Gravatar Email -> 3. Default Icon (null)
                        $profilePhoto = null;
                    ...
                    @if($profilePhoto)
                        {{-- Foto Profil dari Upload DB --}}
                        <img src="{{ $profilePhoto }}" alt="{{ $user->name ?? 'Admin' }}" class="w-full h-full object-cover">
                    @else
                        {{-- Mencoba Ambil Foto Gravatar Email, jika tidak ada tampilkan Default SVG --}}
                        <img src="{{ $gravatarUrl }}" 
                            alt="{{ $user->name ?? 'Admin' }}" 
            ...
            <div x-show="open" 
                ...
                <!-- Section Atas: Avatar Popover & Nama -->
                ...
                <!-- Section Bawah: Informasi Detail Email & No HP -->
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs font-bold text-gray-700 break-all">
                                {{ Auth::user()->email ?? '2300018154@webmail.uad.ac.id' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs font-bold text-gray-700">
                                {{ Auth::user()->no_hp ?? Auth::user()->phone ?? '081234657852' }}
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <aside class="w-64 bg-red-700 text-white flex flex-col justify-between min-h-[calc(100vh-57px)] sticky top-[57px]">
            <div class="py-4">
                <nav class="space-y-1 px-2">
                    ...
                    <!-- Kategori (Ikon Grid Pengelompokan) -->
                    ...
                    <!-- Promo (Ikon Tiket Diskon / Persentase) -->
            ...
            <!-- Tombol Log Out (Triggers Custom Modal) -->
            <div class="p-3 border-t border-red-800" x-data>
                <button type="button" 
                        @click="$dispatch('open-logout-modal')" 
                        class="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-xs font-bold bg-red-900 hover:bg-red-950 transition uppercase tracking-wider text-white cursor-pointer">
    ...
    <div x-data="{ openLogout: false }" 
        ...
        <div x-show="openLogout" 
            ...
            <div class="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl border border-gray-100 relative overflow-hidden"
                ...
                <!-- Ikon Red Circle Sign Out -->
                <div class="w-16 h-16 bg-red-50 text-red-600 rounded-full flex items-center justify-center mx-auto mb-5">
                    <svg class="w-8 h-8 transform rotate-180" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/>
                    </svg>
                </div>
