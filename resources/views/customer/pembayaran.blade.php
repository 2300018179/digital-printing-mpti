...
<div class="max-w-[1000px] mx-auto px-[15px] w-full pt-6 pb-16">
    ...
    {{-- SATU FORM UTAMA UNTUK SELURUH HALAMAN --}}
    <form action="{{ route('proses.simpan-pembayaran') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-[30px] shadow-[0_10px_30px_rgba(0,0,0,0.04)] border border-gray-100 p-6 md:p-10">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
            ...
            <div class="space-y-6 border-b md:border-b-0 md:border-r border-gray-100 pb-8 md:pb-0 md:pr-10">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 tracking-wide font-inder">Selesaikan Pembayaran</h1>
                ...
                <div class="bg-gray-50 rounded-2xl p-4 space-y-4 text-sm">
                    {{-- 1. DAFTAR PRODUK YANG DICHECKOUT --}}
                    <div class="space-y-3">
                        @foreach($checkoutItems as $item)
                            <div class="flex justify-between items-start text-xs text-gray-500">
                                ...
                                <span class="font-medium font-inder text-gray-800 mt-0.5">
                    ...
                    {{-- 2. RINGKASAN TOTAL AKHIR --}}
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs text-gray-500">
                            <span>Total Harga Cetak</span>
                            <span class="font-medium font-inder text-gray-700">Rp {{ number_format($hargaCetak, 0, ',', '.') }}</span>
                        </div>
                        
                        <div class="flex justify-between text-xs text-gray-500">
                            <span>Biaya Layanan</span>
                            <span class="font-medium font-inder text-gray-700">Rp {{ number_format($biayaLayanan, 0, ',', '.') }}</span>
                        </div>
                        
                        <div class="border-t border-dashed border-gray-200 pt-2 flex justify-between font-bold text-base text-gray-800">
                            <span>Grand Total</span>
                            <span class="font-inder text-brandRed">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <label id="label-dp" class="border-2 border-brandRed bg-red-50/30 rounded-xl p-4 flex flex-col justify-between cursor-pointer relative overflow-hidden group transition-all">
                            <input type="radio" name="payment_mode" value="dp" checked onchange="updatePaymentDetails('dp', {{ $uangMuka }}, {{ $grandTotal - $uangMuka }})" class="absolute top-3 right-3 accent-brandRed">
                            <span class="text-xs font-bold text-brandRed tracking-wide uppercase">Uang Muka (DP 50%)</span>
                            <span class="text-lg font-bold text-gray-800 mt-2 font-inder">Rp {{ number_format($uangMuka, 0, ',', '.') }}</span>
                        </label>
                        ...
                        <label id="label-full" class="border border-gray-200 hover:border-gray-300 rounded-xl p-4 flex flex-col justify-between cursor-pointer relative overflow-hidden group transition-all">
                            <input type="radio" name="payment_mode" value="full" onchange="updatePaymentDetails('full', {{ $grandTotal }}, 0)" class="absolute top-3 right-3 accent-brandRed">
                            <span class="text-xs font-bold text-gray-400 tracking-wide uppercase">Bayar Lunas (100%)</span>
                            <span class="text-lg font-bold text-gray-800 mt-2 font-inder">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
                        </label>
                    </div>
                </div>

                {{-- KOTAK INFORMASI JALUR PEMBAYARAN --}}
                <div id="info-pembayaran" class="text-[11px] text-gray-400 bg-amber-50/50 text-amber-700 border border-amber-100 rounded-xl p-3 flex gap-2 items-start">
                    ...
                    <p id="info-text">Jika memilih opsi <strong>DP 50%</strong>, sisa pelunasan sebesar <strong>Rp {{ number_format($grandTotal - $uangMuka, 0, ',', '.') }}</strong> dibayarkan saat Anda mengambil produk di toko.</p>
                </div>
            </div>

            {{-- KOLOM KANAN: QRIS & UPLOAD BUKTI (Frame Putih Dihilangkan) --}}
            <div class="space-y-6">
                
                {{-- Bagian QRIS --}}
                <div class="flex flex-col items-center text-center">
                    <span class="text-xs font-semibold text-gray-400 tracking-wider mb-2">QRIS PAYMENT</span>
                    
                    <img src="{{ asset('icons/qris.png') }}" alt="QRIS Logo" class="h-6 mb-4">
                    
                    <div class="p-3 bg-white border border-gray-100 rounded-2xl shadow-sm mb-4">
                        <img src="{{ asset('icons/qr-code.png') }}" alt="QR Code" class="w-48 h-48">
                    </div>

                    {{-- ID 'badge-nominal' untuk sinkronisasi nominal pembayaran --}}
                    <div class="bg-brandRed text-white px-6 py-2 rounded-full font-bold text-base mb-2 shadow-sm">
                        <span id="badge-nominal">Rp {{ number_format($uangMuka, 0, ',', '.') }}</span>
                    </div>
                    
                    <p class="text-[11px] text-gray-400 mb-4">*Pastikan nominal transfer sesuai angka di atas</p>
                </div>

                {{-- KOTAK UPLOAD BUKTI --}}
                <div class="w-full px-2">
                    <label class="block text-xs font-bold text-gray-700 mb-2 text-left">
                        <i class="fa fa-upload text-brandRed mr-1"></i> Upload Bukti Transfer <span class="text-red-500">*</span>
                    </label>
                    
                    <div class="relative border-2 border-dashed border-gray-200 hover:border-brandRed transition rounded-xl p-4 bg-gray-50/50 text-center cursor-pointer group animate-all" id="upload-container">
                        ...
                        <div class="space-y-1 py-1" id="upload-placeholder">
                            <i class="fa fa-cloud-upload-alt text-2xl text-gray-400 group-hover:text-brandRed transition"></i>
                            <p class="text-xs font-semibold text-gray-600">Pilih Berkas atau Tarik ke Sini</p>
                            <p class="text-[10px] text-gray-400">Format: JPG, PNG, JPEG (Maks. 2MB)</p>
                        </div>

                        <div id="image-preview" class="hidden flex flex-col items-center">
                            <img id="preview-src" src="#" alt="Preview Bukti" class="max-h-24 object-contain rounded-lg border border-gray-100 mb-1">
                            <span id="file-name-text" class="text-[11px] text-gray-500 font-medium truncate max-w-xs"></span>
                ...
                {{-- BUTTON SUBMIT --}}
                <div class="px-2 pt-2">
                    <button type="submit" class="w-full py-3 bg-brandRed text-white font-bold text-sm rounded-full hover:bg-red-700 transition shadow-md">
                        SAYA SUDAH BAYAR
                    </button>
                </div>
...
<script>
    // Konversi Nominal ke Format Rupiah Standard
    function formatRupiah(angka) {
        return new Intl.NumberFormat('id-ID').format(angka);
    }

    // Interaktivitas Perubahan Tipe DP / Full secara Real-Time
    function updatePaymentDetails(type, nominalHarusBayar, nominalSisa) {
        ...
        // Update nilai input hidden agar data tipe pembayaran terkirim dengan benar ke controller
        if(paymentTypeInput) {
        ...
        if (type === 'dp') {
            ...
            infoText.innerHTML = `Jika memilih opsi <strong>DP 50%</strong>, sisa pelunasan sebesar <strong>Rp ${formatRupiah(nominalSisa)}</strong> dibayarkan saat Anda mengambil produk di toko.`;
        } else {
